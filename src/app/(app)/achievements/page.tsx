 ...
 * The sticker sheet.
 *
 * The same sheet the landing page shows, with the account's own state on it:
 * unlocked stickers are printed, locked ones are outlines. Subscribers are
    ...
    <>
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_1.05fr] lg:items-start">
        <Reveal delay={stagger(0)}>
          <StickerSheet tag={`Sheet 01 · ${earned} of ${achievements.length}`}>
        ...
        <Reveal delay={stagger(1)}>
          <Panel flush>
            <PanelHead
              ...
              aside={<Tag tone="ghost">{`${earned} / ${achievements.length}`}</Tag>}
            />
            <ul>
